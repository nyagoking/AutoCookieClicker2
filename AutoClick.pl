...
$sel->execute_script("window.localStorage.setItem('CookieClickerGame', '$savedata')");
while (1) {
...
while ( 1 ) {
    try {
        if ( &click($locator_golden_cookie, "goldenCookie") ) {
            if ( $sel->execute_script("return window.Game.hasBuff('Cookie storm')") ) {
                my $cnt = &click($locator_golden_cookie);
                print "\nclick cookie storming($cnt)";
            }
...
sub click {
    ...
    while (scalar(my @elems = $sel->find_elements($target))) {
        for my $elem (@elems) {
            try {
                $elem->click();
                ++$cnt;
